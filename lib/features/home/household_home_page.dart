...
class _HouseholdHomePageState extends State<HouseholdHomePage> {
  ...
  final _messages = <ConversationMessage>[
    const ConversationMessage(
      ...
      text: '我是家庭管家。你可以直接問我課表，例如「明天有什麼課？」；也可以拍課表匯入。',
    ),
  ];
  ...
  Future<HouseholdConversationService> _conversation() async {
    ...
    return HouseholdConversationService(
      propose: _orchestrator.propose,
      executor: services.actions,
    );
  }
  ...
  Future<void> _confirmSchedule(ScheduleImportDraft draft) async {
    ...
    if (!mounted) return;
    if (confirmed == null) {
      setState(() => _messages.add(const ConversationMessage(
        role: ConversationRole.assistant,
        text: '課表草稿已取消，沒有寫入家庭資料。',
      )));
      return;
    }
    final services = await _servicesFuture;
  ...
  Future<void> _importTimetable() async {
    ...
    await _run(
      ImageAssistantInput(
        ocr: document,
        context: '這是一張家庭成員的學校課表。請依 HOUSEHOLDER_IMPORT_CONTEXT 產生 schedule.import 草稿。',
      ),
      '請幫我匯入這張課表',
    );
  }
  ...
  Future<void> _openSchedule() async {
    ...
    await Navigator.of(context).push<void>(
      MaterialPageRoute(builder: (_) => SchedulePage(repository: services.schedules)),
    );
  }
      ...
      builder: (context) => SafeArea(
        child: Padding(
          ...
          child: SingleChildScrollView(
            child: RecommendedModelInstaller(onInstalled: () {}),
          ),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            key: const ValueKey('schedule-open-button'),
            tooltip: '課表',
                ...
                children: [
                  Expanded(
                    child: FilledButton.tonalIcon(
                      key: const ValueKey('timetable-import-button'),
                      onPressed: _thinking ? null : _importTimetable,
                      icon: const Icon(Icons.document_scanner_outlined),
                      label: const Text('拍照匯入課表'),
                    ),
                  ),
                    ...
                    child: Container(
                      ...
                      child: SelectableText(
                        ...
                        key: index == _messages.length - 1
                            ? const ValueKey('latest-chat-message')
                            : null,
                      ),
                    ),
                  );
